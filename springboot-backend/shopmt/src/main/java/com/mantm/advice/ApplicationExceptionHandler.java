...
public class ApplicationExceptionHandler {
	...
	public ErrorMessage handleInavalidArgument(MethodArgumentNotValidException ex, WebRequest request) {
		ErrorMessage message = new ErrorMessage(HttpStatus.BAD_REQUEST.value(), new Date(0), ex.getMessage(),
				request.getDescription(false));
		return message;
	}
	...
	public ErrorMessage globalExceptionHandler(Exception ex, WebRequest request) {
		ErrorMessage message = new ErrorMessage(HttpStatus.INTERNAL_SERVER_ERROR.value(), new Date(0), ex.getMessage(),
				request.getDescription(false));
